...
/* Where deposits are sent. These are the same two addresses verify-payment
   watches -- a deposit to any other address is ignored there, so the two
   files have to agree or the money arrives and is never credited.
   Overridable by secret so the address can be rotated without a deploy. */
const RECEIVER_BALANCE = Deno.env.get("RECEIVER_BALANCE")
  || "0xDDb438f8c9049f1924e6B6b31d9b0c97540c68C5";
const RECEIVER_PULLS   = Deno.env.get("RECEIVER_PULLS")
  || "0xcce26e367ac0c04e0a9add40e1141d6eabf93b8c";
...
/* Trading. Asks market_open_for, which is the same function the four
   trading functions consult -- so the gate here and the gate in the
   database cannot come to different conclusions about who is allowed in.
   While enabled is false it answers true only for the addresses on
   market_config.test_addresses. */
async function canTrade(addr: string): Promise<boolean> {
  const { data } = await supabase.rpc("market_open_for", { p_address: addr });
  return !!data;
}
...
Deno.serve(async (req) => {
  ...
  try {
    switch (path) {
      ...
      // ---------------------------------------------------------------
      case "list": {
        if (!(await canTrade(addr))) return json({ error: "MARKET_CLOSED" }, 403);
      ...
      // ---------------------------------------------------------------
      // Money. create_topup_order and market_request_withdrawal never
      // look at market_config, so without a gate here deposits would open
      // months before the market does.
      //
      // The tester list applies. A deposit is the one path that cannot be
      // rehearsed against fake data -- it needs a real transfer that
      // verify-payment sees on chain -- so the people testing have to be
      // able to run it. A withdrawal only debits the balance and files a
      // request; nothing leaves until an admin sends it by hand.
      // ---------------------------------------------------------------
      case "topup": {
