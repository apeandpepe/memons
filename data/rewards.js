...
window.MEMONS_REWARDS = {
  ...
  loadServerOwned: function(owned){
    var live={}, imgs={}, self=this, skipped=0;
    ...
    if(!any){ this._live=null; this._imgs=null; return this; }
  ...
  status: function(rkey, claimed){
    var g=this.grades[rkey]; if(!g) return {owned:0,earned:0,claimable:0,max:0};
    var owned=this.ownedKinds(rkey);
    var earned=Math.floor(owned/g.N), max=Math.floor(g.K/g.N);
    return { owned:owned, earned:earned, claimable:Math.max(0, earned-(claimed||0)), max:max };
  }
};
